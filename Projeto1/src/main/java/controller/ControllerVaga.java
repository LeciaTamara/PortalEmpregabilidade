 ...
 */
@WebServlet(urlPatterns = {"/ControllerVaga", "/CadastrarVaga","/mostrarVagas","/mostrarVaga"})
public class ControllerVaga extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if(action.equals("/mostrarVagas")) {
			mostrarVagas(request, response);
	...
	protected void mostrarVagas(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	protected void mostrarVaga(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Vaga> listaVaga = vagaDao.buscarVagasPorEmpresa(nomeEmpresa);
		//ArrayList<Vaga> listaVaga = vagaDao.listaVagas();
		//teste de recebimento
		for(int i = 0; i < listaVaga.size(); i++) {
			System.out.println(listaVaga.get(i).getTitulo());
		...
		request.setAttribute("listaVaga", listaVaga);
		RequestDispatcher rd = request.getRequestDispatcher("VagasOfertadas.jsp");
		rd.forward(request, response);
	}
